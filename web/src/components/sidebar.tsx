...
import { Inbox, Radar, Plus } from "lucide-react";
...
export function Sidebar({
  ...
  return (
    <aside className="flex h-screen w-60 flex-shrink-0 flex-col border-r border-border-light bg-surface">
      ...
      <nav className="flex flex-col gap-0.5 px-2">
        ...
        {workspaces.map((ws) => {
          ...
          return (
            <Link key={ws.id} href={`/workspace/${ws.id}`} className={navItemClass(active)}>
